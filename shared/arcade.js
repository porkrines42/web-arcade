(function () {
  ...
  function openSettings(opener) {
    ...
    modal.querySelector('#settings-install').hidden = !installPrompt; modal.querySelector('[data-reset="pong"]').hidden = !localStorage.getItem(PONG_KEY) && !localStorage.getItem(STATS_KEY) && !localStorage.getItem(LEGACY_DIFFICULTY); modal.hidden = false; document.body.classList.add('modal-open'); modal.querySelector('.settings-close').focus(); modal._opener = opener;
  }
  function settingsModal() {
    ...
    modal.querySelectorAll('[data-reset]').forEach(button => button.addEventListener('click', () => { const kind = button.dataset.reset; const message = kind === 'settings' ? 'Reset audio, visuals, theme, and gameplay defaults? Pong preferences will stay.' : kind === 'pong' ? 'Reset saved Pong preferences and statistics, including its difficulty? This cannot be undone.' : 'Clear all saved Web Arcade settings, Pong preferences, and statistics on this device? This cannot be undone.'; if (!window.confirm(message)) return; if (kind === 'settings') localStorage.removeItem(SETTINGS_KEY); if (kind === 'pong') { localStorage.removeItem(PONG_KEY); localStorage.removeItem(STATS_KEY); localStorage.removeItem(LEGACY_DIFFICULTY); } if (kind === 'all') Object.keys(localStorage).filter(key => key.startsWith('webArcade.') || key.startsWith('web-arcade-')).forEach(key => localStorage.removeItem(key)); apply(); openSettings(modal._opener); }));
  }
