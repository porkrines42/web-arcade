(function () {
  ...
  function settings() { return Object.assign({}, defaults, read(SETTINGS_KEY, {})); }
  ...
  function openSettings(opener) {
    ...
    modal.querySelector('#settings-install').hidden = !installPrompt; modal.querySelector('[data-reset="pong"]').hidden = !pong().difficulty && !localStorage.getItem(LEGACY_DIFFICULTY); modal.hidden = false; document.body.classList.add('modal-open'); modal.querySelector('.settings-close').focus(); modal._opener = opener;
  }
  function settingsModal() {
    ...
    modal.innerHTML = `<div class="settings-dialog"><header><div><p class="settings-kicker">WEB ARCADE</p><h2 id="settings-title">Settings Center</h2></div><button class="settings-close" type="button" aria-label="Close settings">×</button></header><div class="settings-content"><section><h3>Audio</h3><label class="setting-switch"><input id="setting-sound" type="checkbox"><span>Global sound</span></label><label for="setting-volume">Global volume <output id="volume-value">70%</output></label><input id="setting-volume" type="range" min="0" max="100" step="1"></section><section><h3>Visuals</h3><label for="setting-motion">Animation</label><select id="setting-motion"><option value="system">System default</option><option value="full">Full</option><option value="reduced">Reduced</option><option value="off">Off</option></select><p class="setting-help">System default follows your browser’s reduced-motion preference.</p><label for="setting-theme">Theme</label><select id="setting-theme"><option value="arcade-dark">Arcade Dark</option><option value="midnight">Midnight</option><option value="high-contrast">High Contrast</option></select></section><section><h3>Gameplay defaults</h3><label for="setting-difficulty">Default Pong difficulty</label><select id="setting-difficulty"><option value="easy">Easy</option><option value="normal">Normal</option><option value="hard">Hard</option></select><p class="setting-help">Used only until Pong has its own saved difficulty.</p></section><section><h3>Data controls</h3><p class="setting-help">Reset settings removes audio, visual, theme, and default difficulty choices. Pong preferences are kept.</p><div class="settings-actions"><button type="button" data-reset="settings">Reset settings</button><button type="button" data-reset="pong">Reset Pong preferences</button><button type="button" class="danger" data-reset="all">Clear all Web Arcade data</button></div></section><section><h3>App information</h3><dl class="app-info"><div><dt>Version</dt><dd>v${VERSION}</dd></div><div><dt>Connection</dt><dd id="app-network"></dd></div><div><dt>Standalone</dt><dd id="app-standalone"></dd></div></dl><button id="settings-install" type="button" hidden>Install app</button><p class="setting-help">On iPhone or iPad, use Safari’s Share button, then choose <strong>Add to Home Screen</strong>.</p></section></div></div>`;
    ...
    modal.querySelectorAll('[data-reset]').forEach(button => button.addEventListener('click', () => { const kind = button.dataset.reset; const message = kind === 'settings' ? 'Reset audio, visuals, theme, and gameplay defaults? Pong preferences will stay.' : kind === 'pong' ? 'Reset saved Pong preferences, including its difficulty? This cannot be undone.' : 'Clear all saved Web Arcade settings, Pong preferences, and statistics on this device? This cannot be undone.'; if (!window.confirm(message)) return; if (kind === 'settings') localStorage.removeItem(SETTINGS_KEY); if (kind === 'pong') { localStorage.removeItem(PONG_KEY); localStorage.removeItem(LEGACY_DIFFICULTY); } if (kind === 'all') Object.keys(localStorage).filter(key => key.startsWith('webArcade.') || key.startsWith('web-arcade-')).forEach(key => localStorage.removeItem(key)); apply(); openSettings(modal._opener); }));
  }
